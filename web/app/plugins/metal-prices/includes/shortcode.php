...
<div class="text-white-cloud bg-green-primary" data-gsap>
    <div class="container mx-auto">
        <div data-anim data-position="0.25" data-from='{"x": -100,"autoAlpha":0}'
            data-to='{"x": 0,"autoAlpha":1,"duration": 1.25}'>
            <h2 id="current-price-title">Prix courant de <?php echo esc_html($metal_name); ?>:
                <?php echo esc_html($current_price['price']); ?> €</h2>
        ...
        <div class="metal-prices" data-anim data-position="0.75" data-from='{"y": 100,"autoAlpha":0}'
            data-to='{"y": 0,"autoAlpha":1,"duration": 1.25}'>
        ...
        function fetchAndRenderChart(metal, period) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#current-price-title').text(
                        `Prix courant ${metalName}: ${data[data.length - 1].price} €`);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var previousPrice = data.previous ? data.previous.price :
                    currentPrice; // Handle the case where there's no previous price
                    ...
                    var priceChangeFormatted = (priceChange >= 0 ? '+' : '') + priceChange.toFixed(
                        2);
                    var percentageChangeFormatted = (priceChange >= 0 ? '+' : '') + percentageChange
                        .toFixed(2) + '%';

                    $('#price-change').text(
                        `(${priceChangeFormatted} €, ${percentageChangeFormatted})`).attr(
                        'class', changeClass);
                },
                error: function (jqXHR, textStatus, errorThrown) {
